...
public class TryCatchExecutor {
    public static<T> T execute(Supplier<T> supplier, String notFoundMaessage, String wrongPageSize){
        try {
            ...
            if (result instanceof Page && ((Page) result).isEmpty()){
                throw new NotFoundException(wrongPageSize);
            }
            return result;
        }catch (MaxDataException | NotFoundException | DataIntegrityViolationException e){
            throw e;
        }catch (Exception e){
